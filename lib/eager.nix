...
in
{
  propagateEager =
    ...
    let
      ...
      # EVERY ACCUMULATOR FIELD IS FORCED PER ROUND, and the discipline is per-FIELD.
      # `enqueued` is read by the control flow in strict position and is thereby forced
      # already; `settled` and `affected` are never read by it, so each would chain one
      # `//` or `++` thunk per cone node and then overflow the C stack when something
      # finally forces the chain — an abort no call-depth setting bounds and `tryEval`
      # does not catch. Forcing some of the fields measures the same as forcing none, so
      # all three are named rather than the two that look risky.
      forceRound = st: builtins.seq st.settled (builtins.seq st.enqueued (builtins.seq st.affected st));
      step =
        st: id:
