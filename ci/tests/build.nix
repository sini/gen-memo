...
in
{
  flake.tests.build = {
    ...
    test-fixpoint-condensation-producer-before-consumer = {
      expr =
        let
          ...
          # and does not survive being read across a library boundary.
    ...
    # 5b. The ordering gate above is only evidence if the two SCCs are actually distinct
    #     and both actually appear in `bottomUp`. A partition that mapped everything to
    #     one tag would satisfy `iB < iA` vacuously — or rather would not, but a
    #     `findFirstIndex` returning the default -1 for both would give `-1 < -1` = false
    #     in one direction and could give a spurious true in the other if only one
    #     resolved. Pinning the shape makes the gate's premise visible.
    test-fixpoint-condensation-two-distinct-strata = {
      expr =
        ...
        in
        {
          ...
          bothPresent = builtins.all (t: builtins.elem t cond.bottomUp) [
            cond.sccOf.a1
            cond.sccOf.b1
          ];
        };
