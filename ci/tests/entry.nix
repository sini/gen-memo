...
#
# Every other cell here takes `genMemo` from `ci/flake.nix`, which builds it with `import ../lib`
...
in
{
  # The two entry paths are ONE library. `genMemo` is built from ci's flake inputs, `dispatched`
  ...
  # genMemo` is not the alternative either — `./lib`'s surface carries function-valued members, and
