...
in
{
  flake.tests."provenance" = {
    ...
    # ===== whyNot: the negative operator query, TOTAL over all three verdicts =====
    # recomputed ⇒ a record naming the verdict, NOT an absence. The record is what makes
    # "it was recomputed, nothing was avoided" readable; the `null` this used to answer
    ...
    # THE TOTALITY ITSELF, as one cell rather than three coincidences: every verdict
    # answers with the same field set, so a caller reads one shape and never branches on
    # whether an answer arrived.
    test-whyNot-shape-is-total = {
      expr =
        builtins.all
          (
            r:
