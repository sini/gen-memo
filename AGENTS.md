...
Both are empty at this revision: 292 distinct names on each side, over a run of 294 cells (two names
...
lower-case-only form truncates each at the capital, so it reports **253** names against the correct
**292**. A reconciliation run with it shows 12 spurious mismatches on one side and hides 51 real ones
on the other.
