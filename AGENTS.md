...
| The static attribute-dependency schedule, and the **cold** fold                                                  | **Not this library, and not one successor either.** The **warm** fold and override cone moved here; the schedule and the cold path have NOT moved, and their destinations are the query-gate home and the evaluator respectively — neither is this library. **`gen-resolve` DISSOLVED rather than moving as one library.** ADR-0008 §4 orphans the repository as reference and the 2026-09-02 transfer ruling dispositioned its eleven exports ROW BY ROW rather than as one content block (map at `den-hoag-p3y9`); take no new dependency on it                                                                                                                                                |
...
| Trap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A DERIVATION IS ONLY A HASHED NODE VALUE BECAUSE THE WALK STOPS WHERE THE PROJECTION STOPS — DO NOT REMOVE EITHER SHORT-CIRCUIT.** A derivation is SELF-REFERENTIAL, so a walk that descends into one never finishes. `project` recognises the derivation shape at EVERY position BEFORE descending and yields a tagged `{ __drvPath = …; }` record, which is what `hashOf` is handed; `hashGuarded`'s bounded walk reads the raw value and treats the same shape as a leaf (`isRecord`). Remove the walk's stop and nothing aborts any more — every derivation-valued node goes SILENTLY always-dirty instead, which only the reuse cells notice. A ROOT-only test is not a smaller version of either — it leaves the class expressible one attribute deeper | `(builtins.elemAt drv.all 0) == drv` ⇒ **true**, and `drv`'s keys are `all args builder drvAttrs drvPath name out outPath outputName system type`. With the walk's derivation stop removed (both `isDrv` tests replaced by `true`/`false`), `classify drv` and `classify { pkg = drv; }` ⇒ `"exhausted"` and `hashGuarded hashOf { pkg = drv; } != null` ⇒ `false`; the same expression on the shipped walk ⇒ `"finite"`, `"finite"`, `true` (same run, upstream Nix 2.34.8). `ci/tests/hash.nix` pins root / nested-in-attrs / nested-in-list / three-level projection, the marker-without-`drvPath` fall-through, and the function-beside-a-derivation discrimination; `ci/tests/byte-parity.nix` drives derivation-valued nodes through the plane and asserts the trace hash was really taken. ONE RESIDUE, live: the projection is NOT injective (a literal `{ __drvPath = …; }` still collides, and the direction is false-clean). The general non-well-founded class is no longer a residue — it falls back to always-dirty (README, Limitations) |
...
| **A bare-leaf nix-unit target reports `0/0` — a false pass.** Establish a suite is non-vacuous before reading its green                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | This suite is 5 cells across 2 suites, and the purity scan carries its own in-suite positive control (`test-detector-catches-injected-violation`, which asserts the scan returns the injected token's violation on a source list carrying a planted tether) so its absence claim cannot pass by a dead predicate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
