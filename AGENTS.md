...
| Fold and algebra primitives, intensional functions                                                               | `gen-algebra` — "gen-algebra: pure Nix algebra — records, intensional functions, either"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Trap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A DERIVATION IS ONLY A HASHED NODE VALUE BECAUSE OF THE ADMISSION PROJECTION — DO NOT REMOVE THE SHORT-CIRCUIT.** A derivation is SELF-REFERENTIAL, so a walk that descends into one never finishes. `project` recognises the derivation shape at EVERY position BEFORE descending and yields a tagged `{ __drvPath = …; }` record; `hashGuarded` runs both the bounded walk and `hashOf` over that image. Remove the projection's stop and nothing aborts any more — the walk exhausts its depth bound inside every derivation, and every derivation-valued node goes SILENTLY always-dirty instead, which only the reuse cells notice. A ROOT-only test is not a smaller version of this — it leaves the class expressible one attribute deeper | `(builtins.elemAt drv.all 0) == drv` ⇒ **true**, and `drv`'s keys are `all args builder drvAttrs drvPath name out outPath outputName system type`. With the projection's stop removed (`isDrv` replaced by `_: false`), `classify drv` and `classify { pkg = drv; }` ⇒ `"exhausted"` and `hashGuarded hashOf { pkg = drv; } != null` ⇒ `false`; the same expression on the shipped walk ⇒ `"finite"`, `"finite"`, `true` (same run, upstream Nix 2.34.8). `ci/tests/hash.nix` pins root / nested-in-attrs / nested-in-list / three-level projection, the marker-without-`drvPath` fall-through, and the function-beside-a-derivation discrimination; `ci/tests/byte-parity.nix` drives derivation-valued nodes through the plane and asserts the trace hash was really taken. ONE RESIDUE, live: the projection is NOT injective (a literal `{ __drvPath = …; }` still collides, and the direction is false-clean). The general non-well-founded class is no longer a residue — it falls back to always-dirty (README, Limitations) |
