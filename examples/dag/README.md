...
A runnable, docs-as-code proof of the gen-memo thesis: **sound intra-eval
...
1. **Located cycle.** A variant where the network base depends on the gateway
   ...
   (`cycleIsLocatedBlame = true`) — gen-memo runs `graph.cycles` as a
...
gen-memo is the **rebuilder** dimension of Mokhov 2018 made explicit: a
...
gen-memo externalizes the dependency graph and result store as values you can
